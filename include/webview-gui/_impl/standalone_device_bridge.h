...
function installNativeReceiver(){const token=bridgeToken();if(!/^[0-9a-f]{64}$/.test(token))return false;const name="_WebviewGui_send_"+token,deliver=window[name];if(typeof deliver!=="function")return false;window[name]=base64=>{const bytes=decodeNative(base64);if(acceptSnapshot(bytes))return;deliver(base64);};return true;}
ensureUi();
