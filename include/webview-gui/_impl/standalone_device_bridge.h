...
public:
    ...
    bool provideResource(const char *path, WebviewGui::Resource &resource) noexcept {
        if (!path || std::strcmp(path, kScriptPath) != 0 || !host_)
            return false;
    ...
    void injectIntoHtml(WebviewGui::Resource &resource) noexcept {
        // Do not gate injection on the first host-extension lookup. Some wrapper
        // hosts finish publishing private host extensions during GUI startup. The
        // injected control stays hidden until a valid standalone snapshot arrives,
        // so DAW/non-standalone hosts remain visually unchanged.
        if (!host_ || resource.mediaType.rfind("text/html", 0) != 0 || resource.bytes.empty())
            return;
    ...
    bool receive(const unsigned char *bytes, size_t size, Sender &&send) noexcept {
        ...
        // The discovery probe belongs to this bridge even before the wrapper has
        // exposed the private extension. Consume it so it never leaks into the
        // plug-in's own WebView protocol, and retry resolution on the next probe.
...
#wvg-io-button[hidden]{display:none!important}
...
function acceptSnapshot(bytes){if(!bytes||bytes.byteLength<4||bytes[0]!==0x57||bytes[1]!==0x56||bytes[2]!==0x44||bytes[3]!==0x4a)return false;try{state=JSON.parse(dec.decode(bytes.subarray(4)));render();}catch(_){}return true;}
...
function probe(){if(state||probeAttempts++>=20)return;installNativeReceiver();request();if(!state)setTimeout(probe,50);}
ensureUi();
probe();
})());)js";
};
