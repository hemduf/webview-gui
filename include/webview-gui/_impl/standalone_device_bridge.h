...
function acceptSnapshot(bytes){ if(!bytes||bytes.byteLength<4||bytes[0]!==0x57||bytes[1]!==0x56||bytes[2]!==0x44||bytes[3]!==0x4a)return false;try{state=JSON.parse(dec.decode(bytes.subarray(4)));if(document.body)render();else document.addEventListener("DOMContentLoaded",render,{once:true});}catch(_){return false;}return true; }
