...
| `clap.params` | Partial | Fine Tune, Master Gain, Waveform, Coarse Tune, and Pan are published with stable module-lifetime cookies. Fine Tune keeps its qualified global and full `(note_id, port_index, channel, key)` polyphonic `PARAM_VALUE`/`PARAM_MOD` addressing. Master Gain is global-only: sample-accurate base automation and modulation compose in dB without overwriting the host-visible base, while targeted per-note statements are ignored. Waveform is a global stepped `Sine / Saw / Square` parameter with lock-free retained host state and bounded text conversion. Coarse Tune is a global stepped `-48..+48` semitone NOTE_ON default: host-facing values use the pinned CLAP truncation rule, active/inactive `params.flush()` route through the same engine path, and existing voice generations retain their NOTE_ON coarse snapshot. Pan exposes the already-qualified full polyphonic addressing path over `-1..+1`; global host state is retained lock-free while targeted value/modulation remains voice-local and composes with CLAP PAN note expression without overwriting the global base. Event handling remains parameter-ID authoritative and accepts the `nullptr` cookie that CLAP hosts are allowed to provide. Additional parameter rows remain #32 work. |
| `clap.state` | Implemented | The bounded 36-byte version-5 payload persists Fine Tune, Master Gain, Waveform, Coarse Tune, and Pan base values without serializing ephemeral modulation/note-expression state. Version-1 and version-2 24-byte payloads, version-3 28-byte payloads, and version-4 32-byte payloads remain loadable, with missing newer fields defaulting to 0 dB / Sine / 0 semitones / centered Pan as appropriate. Malformed, truncated, trailing, non-finite, out-of-range, or invalid stepped data is rejected before host snapshots are mutated. |
...
| `clap.remote-controls/2` | Partial | Two stable non-preset pages are published through both pinned IDs: `Oscillator / Tuning` maps Fine Tune, Waveform, and Coarse Tune, while `Output / Performance` maps Master Gain and Pan. Every unused slot is `CLAP_INVALID_ID`. Additional Oscillator/Filter/Envelope/Performance slots follow the remaining parameter surface. |
