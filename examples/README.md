...
| `clap.params` | Partial | Fine Tune, Master Gain, Waveform, Coarse Tune, Pan, Filter Cutoff, Filter Resonance, Filter Env, and Amp Level are published with stable module-lifetime cookies. Fine Tune keeps its qualified global and full `(note_id, port_index, channel, key)` polyphonic `PARAM_VALUE`/`PARAM_MOD` addressing. Master Gain is global-only: sample-accurate base automation and modulation compose in dB without overwriting the host-visible base, while targeted per-note statements are ignored. Waveform is a global stepped `Sine / Saw / Square` parameter with lock-free retained host state and bounded text conversion. Coarse Tune is a global stepped `-48..+48` semitone NOTE_ON default: host-facing values use the pinned CLAP truncation rule, active/inactive `params.flush()` route through the same engine path, and existing voice generations retain their NOTE_ON coarse snapshot. Pan exposes the already-qualified full polyphonic addressing path over `-1..+1`; global host state is retained lock-free while targeted value/modulation remains voice-local and composes with CLAP PAN note expression without overwriting the global base. Filter Cutoff is published at stable ID 1004 / host index 5 over `20..20000` Hz, retains its global base through lock-free snapshots, routes active/inactive `params.flush()` through the qualified RT-safe cutoff path, and keeps targeted value/modulation voice-local while composing with CLAP BRIGHTNESS. Filter Resonance remains stable at ID 1005 / host index 6 over `0..0.99`, reuses the qualified fixed-capacity polyphonic routing and RT-safe TPT coefficient handoff, retains its host-visible global base lock-free, and leaves targeted value/modulation voice-local. Filter Envelope Amount is appended at stable ID 1010 / host index 7 over `-1..+1`; it uses the same full polyphonic address tuple, applies active automation/modulation through the precomputed RT-safe envelope-cutoff handoff, retains the global host-visible base lock-free, and composes with Filter Cutoff, CLAP BRIGHTNESS, and the voice-local envelope without serializing transient modulation. Amp Level is appended at stable ID 1012 / host index 8 over `0..1`; its global base is retained through a lock-free snapshot, active/inactive `params.flush()` reuse the qualified fixed-capacity voice path, and targeted value/modulation remains generation-local while composing multiplicatively with CLAP VOLUME, EXPRESSION, and PRESSURE. Event handling remains parameter-ID authoritative and accepts the `nullptr` cookie that CLAP hosts are allowed to provide. Additional parameter rows remain #32 work. |
| `clap.state` | Implemented | The bounded 52-byte version-9 payload persists Fine Tune, Master Gain, Waveform, Coarse Tune, Pan, Filter Cutoff, Filter Resonance, Filter Env, and Amp Level base values without serializing ephemeral modulation/note-expression state. Version-1 and version-2 24-byte payloads, version-3 28-byte payloads, version-4 32-byte payloads, version-5 36-byte payloads, version-6 40-byte payloads, version-7 44-byte payloads, and version-8 48-byte payloads remain loadable; Filter Cutoff defaults to 6000 Hz when loading pre-v6 state, Filter Resonance defaults to 0 when loading pre-v7 state, Filter Env defaults to 0 when loading pre-v8 state, and Amp Level defaults to 1 when loading pre-v9 state while earlier fields keep their existing migration defaults. Malformed, truncated, trailing, non-finite, out-of-range, or invalid stepped data is rejected before host snapshots are mutated. |
...
| `clap.remote-controls/2` | Partial | Three stable non-preset pages are published through both pinned IDs: `Oscillator / Tuning` maps Fine Tune, Waveform, and Coarse Tune; `Output / Performance` maps Master Gain, Pan, and Amp Level; and `Filter / Tone` maps Filter Cutoff, Filter Resonance, and Filter Env. Every unused slot is `CLAP_INVALID_ID`. Additional Oscillator/Filter/Envelope/Performance slots follow the remaining parameter surface. |
