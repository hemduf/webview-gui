...
| `clap.remote-controls/2` | Partial | Two stable non-preset pages are published through both pinned IDs: `Oscillator / Tuning` maps Fine Tune and Waveform, while `Output / Performance` maps Master Gain. Every unused slot is `CLAP_INVALID_ID`. Additional Oscillator/Filter/Envelope/Performance slots follow the remaining parameter surface. |
