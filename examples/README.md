...
| `clap.params` | Partial | Fine Tune and Master Gain are published with stable module-lifetime cookies. Fine Tune keeps its qualified global and full `(note_id, port_index, channel, key)` polyphonic `PARAM_VALUE`/`PARAM_MOD` addressing. Master Gain is global-only: sample-accurate base automation and modulation compose in dB without overwriting the host-visible base, while targeted per-note statements are ignored. Event handling remains parameter-ID authoritative and accepts the `nullptr` cookie that CLAP hosts are allowed to provide. Additional parameter rows remain #32 work. |
| `clap.state` | Implemented | The bounded 24-byte version-2 payload persists the Fine Tune and Master Gain base values without serializing ephemeral modulation/note-expression state. Version-1 Fine-Tune-only payloads remain loadable with Master Gain defaulting to 0 dB. Malformed, truncated, trailing, non-finite, or out-of-range data is rejected before host snapshots are mutated. |
...
| `clap.remote-controls/2` | Partial | Two stable non-preset pages are published through both pinned IDs: `Oscillator / Tuning` maps Fine Tune and `Output / Performance` maps Master Gain. Every unused slot is `CLAP_INVALID_ID`. Additional Oscillator/Filter/Envelope/Performance slots follow the remaining parameter surface. |
