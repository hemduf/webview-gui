...
| `clap.params` | Partial | Fine Tune, Master Gain, Waveform, Coarse Tune, Pan, Filter Cutoff, Filter Resonance, Filter Env, and Amp Level keep their already-qualified host indices `0..8`. Amp Attack, Amp Decay, Amp Sustain, and Amp Release are appended at host indices 9..12 with stable IDs 1006..1009, preserving existing project automation indices even though the stable IDs numerically precede Filter Env, Pan, and Amp Level. The four Amp Envelope parameters are global-only NOTE_ON defaults: active/inactive `params.flush()` reuse the ordered parameter handoff, finite out-of-range or targeted per-note statements are ignored, seconds are converted deterministically to sample counts at the active sample rate, and already-running generations retain their coherent NOTE_ON ADSR snapshot. Fine Tune keeps its qualified global and full `(note_id, port_index, channel, key)` polyphonic `PARAM_VALUE`/`PARAM_MOD` addressing. Master Gain is global-only: sample-accurate base automation and modulation compose in dB without overwriting the host-visible base, while targeted per-note statements are ignored. Waveform is a global stepped `Sine / Saw / Square` parameter with lock-free retained host state and bounded text conversion. Coarse Tune is a global stepped `-48..+48` semitone NOTE_ON default. Pan, Filter Cutoff, Filter Resonance, Filter Env, and Amp Level retain their full qualified polyphonic addressing/composition paths. Event handling remains parameter-ID authoritative and accepts the `nullptr` cookie that CLAP hosts are allowed to provide. |
...
| `clap.tail` | Implemented | The Release parameter converted to samples at the active sample rate is published through a lock-free tail snapshot. An active audio-thread Release increase updates the finite tail immediately and calls `host.tail.changed()` when the value changes. A Release decrease remains conservative while any older voice generation is active because those voices retain their NOTE_ON release snapshot; the shorter tail is published after the active set drains. Inactive/main-thread parameter changes never call the audio-thread-only tail notification. No delay/reverb/feedback source extends the tail. |
| `clap.remote-controls/2` | Partial | Four stable non-preset pages are published through both pinned IDs: `Oscillator / Tuning` maps Fine Tune, Waveform, and Coarse Tune; `Output / Performance` maps Master Gain, Pan, and Amp Level; `Filter / Tone` maps Filter Cutoff, Filter Resonance, and Filter Env; and `Amp Envelope / ADSR` maps Attack, Decay, Sustain, and Release. Every unused slot is `CLAP_INVALID_ID`. Additional Oscillator/Filter/Performance slots follow any remaining parameter surface. |
