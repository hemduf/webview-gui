...
| `clap.params` | Partial | Fine Tune, Master Gain, Waveform, Coarse Tune, Pan, Filter Cutoff, and Filter Resonance are published with stable module-lifetime cookies. Fine Tune keeps its qualified global and full `(note_id, port_index, channel, key)` polyphonic `PARAM_VALUE`/`PARAM_MOD` addressing. Master Gain is global-only: sample-accurate base automation and modulation compose in dB without overwriting the host-visible base, while targeted per-note statements are ignored. Waveform is a global stepped `Sine / Saw / Square` parameter with lock-free retained host state and bounded text conversion. Coarse Tune is a global stepped `-48..+48` semitone NOTE_ON default: host-facing values use the pinned CLAP truncation rule, active/inactive `params.flush()` route through the same engine path, and existing voice generations retain their NOTE_ON coarse snapshot. Pan exposes the already-qualified full polyphonic addressing path over `-1..+1`; global host state is retained lock-free while targeted value/modulation remains voice-local and composes with CLAP PAN note expression without overwriting the global base. Filter Cutoff is published at stable ID 1004 / host index 5 over `20..20000` Hz, retains its global base through lock-free snapshots, routes active/inactive `params.flush()` through the qualified RT-safe cutoff path, and keeps targeted value/modulation voice-local while composing with CLAP BRIGHTNESS. Filter Resonance is appended at stable ID 1005 / host index 6 over `0..0.99`, reuses the qualified fixed-capacity polyphonic routing and RT-safe TPT coefficient handoff, retains its host-visible global base lock-free, and leaves targeted value/modulation voice-local. Filter Envelope Amount ID 1010 is now retained in the internal fixed-capacity parameter model, including global/targeted value and modulation storage, but is intentionally **not yet advertised** through `clap.params`: DSP application, host snapshots/state and Remote Controls remain separate #32 increments. Event handling remains parameter-ID authoritative and accepts the `nullptr` cookie that CLAP hosts are allowed to provide. Additional parameter rows remain #32 work. |
| `clap.state` | Implemented | The bounded 44-byte version-7 payload persists Fine Tune, Master Gain, Waveform, Coarse Tune, Pan, Filter Cutoff, and Filter Resonance base values without serializing ephemeral modulation/note-expression state. Version-1 and version-2 24-byte payloads, version-3 28-byte payloads, version-4 32-byte payloads, version-5 36-byte payloads, and version-6 40-byte payloads remain loadable; Filter Cutoff defaults to 6000 Hz when loading pre-v6 state and Filter Resonance defaults to 0 when loading pre-v7 state while earlier fields keep their existing migration defaults. Malformed, truncated, trailing, non-finite, out-of-range, or invalid stepped data is rejected before host snapshots are mutated. |
...
| `clap.remote-controls/2` | Partial | Three stable non-preset pages are published through both pinned IDs: `Oscillator / Tuning` maps Fine Tune, Waveform, and Coarse Tune; `Output / Performance` maps Master Gain and Pan; and `Filter / Tone` maps Filter Cutoff and Filter Resonance. Every unused slot is `CLAP_INVALID_ID`. Additional Oscillator/Filter/Envelope/Performance slots follow the remaining parameter surface. |
