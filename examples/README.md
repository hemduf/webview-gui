...
| `clap.remote-controls/2` | Partial | Three stable non-preset pages are published through both pinned IDs: `Oscillator / Tuning` maps Fine Tune, Waveform, and Coarse Tune; `Output / Performance` maps Master Gain and Pan; and `Filter / Tone` maps Filter Cutoff, Filter Resonance, and Filter Env. Every unused slot is `CLAP_INVALID_ID`. Additional Oscillator/Filter/Envelope/Performance slots follow the remaining parameter surface. |
